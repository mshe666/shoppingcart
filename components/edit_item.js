...
export default class edit_item extends Component {
    ...
    _handleEdit() {
        ...
        console.log("name=" + this.state.name + ",quantity=" + this.state.quantity + ",description=" + this.state.description + ",image=" + this.state.image);

        firebase.database().ref('items/' + uuid).update(item).then(() => {
            console.log('UPDATED!');
        }).catch(error => {
            console.log(error);
        })
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
                <View style={style.addItemContainer}>
                    ...
                    <View style={style.addButtonContainer}>
                        <TouchableHighlight style={style.addButton} onPress={() => this._handleEdit()}>
